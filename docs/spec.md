...
byte-aligned (there is no bit packing). The current container format is version 3.
...
Frames (one or more, repeated until is_last = 1)
  ...
  tag              HMAC-SHA256 over the AAD      32 bytes
```
...
The `mac id` selects the MAC. Currently the only value is `1`, HMAC-SHA256, which
produces a 32-byte tag. The MAC key is 32 bytes.
...
Each frame's tag is `HMAC-SHA256(mac_key, AAD)`, where the authenticated data is
built as follows:
...
- The **header bytes**, bound into the first frame only, authenticate every
  parameter (variant, KDF id and settings, salt, tweak, IV). The stored tag was
...
the layout is a version bump (`format::VERSION`).
