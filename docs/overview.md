...
flowchart TB
    ...
    ctr --> ct["ciphertext"]
    ct --> hmac["HMAC-SHA256"]
    mackey --> hmac --> tag["authentication tag"]
```
...
- **Differential tests**: dorado compared against an independent implementation
  (the RustCrypto `threefish` crate) over thousands of random inputs.
- **CTR tests**: anchored to the verified cipher, plus round-trips at awkward
  lengths.
- **Unit tests** for the file header, the KDFs, the MAC, and the chunk counter.
...
The natural next step is to complete the lineage and build Skein, the hash function
Threefish was designed for, by implementing its chaining mode (UBI) on top of the
cipher. A Skein-based MAC could then replace HMAC for a single-primitive design.
Other candidates: making the core run without the standard library, and
implementing the RustCrypto `cipher` traits for ecosystem interop.
