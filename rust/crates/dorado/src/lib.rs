...
//!
//! // CTR mode handles any length, and the same call decrypts.
//! let iv = [0u8; 32];
//! let mut data = b"any length, not just one block".to_vec();
//! cipher.ctr_apply(&iv, &mut data);
//! cipher.ctr_apply(&iv, &mut data);
//! assert_eq!(&data[..], b"any length, not just one block");
//! ```
...
// Wipe the expanded key schedule (the secret-derived `ek`) and the extended
// tweak from memory when a cipher is dropped. Gated on the `zeroize` feature so
// the bare core stays dependency-free. The cipher state in `encrypt`/`decrypt`
// is a short-lived stack buffer and not persisted, so the schedule is the only
// long-lived secret to clear.
#[cfg(feature = "zeroize")]
mod zeroize_impls {
