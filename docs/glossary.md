...
and BLAKE3 all support this, which is why dorado's three MACs are interchangeable:
each is a keyed hash that maps the message to an unpredictable fixed-size tag. (This
is different from a one-time MAC like Poly1305, whose key must never be reused
across two messages.)

**HMAC.** A specific, well-studied way to build a MAC out of a plain hash function
(here SHA-256). You compute `HMAC(key, data)` and get a tag. It is one of dorado's
three MAC choices.

**Skein-MAC.** Skein's native keyed mode: rather than wrapping a hash the way HMAC
does, Skein takes the key directly through its chaining mode to produce a tag. It is
dorado's default MAC, keeping the construction on a single Threefish-based primitive.
...
is used for integrity checks and fingerprints. dorado implements two from scratch,
Skein-512 and BLAKE3, and exposes Skein-512 as the standalone `gyotaku` command-line
tool (the Skein counterpart of `sha256sum`).
...
the car built around it. dorado implements Skein-512 on its Threefish-512 and uses
it two ways: as the default authentication MAC and as the `gyotaku` hashing tool.
