...
- The `dorado` CLI suppresses core dumps at startup (`RLIMIT_CORE` = 0, via
  `golang.org/x/sys/unix` in the `secure` package), so a crash cannot leave the
  password or derived keys in a core file. `mlock` keeps those pages out of
  swap but not out of a core dump, so this complements it. Best-effort,
  Unix-only, a no-op elsewhere. See the [Core changelog](../CHANGELOG.md) for
  the cross-port rationale.
- Cross-compat password-container fixtures generated by the Rust CLI
  (`engine/testdata/`: every KDF, MAC, and variant, plus labeled and
  multi-frame files) with decrypt tests in `engine/crosscompat_test.go`.
  Until now the Go suite verified cross-compatibility only via the shared
  hex vectors, with no committed `.mahi` containers.
