...
- **Cross-compat is now verified in both directions and with full fixture
  ...
  spanning every KDF, MAC, and cipher variant (Go and TS gained their first
  committed containers; Haskell and C++ gained the missing Threefish-1024
  file). Reverse: the Rust suite now decrypts a container generated by each
  other port's own encrypt path (see
  [`rust/CHANGELOG.md`](rust/CHANGELOG.md)). Until now all fixtures were
  Rust-generated and Go/TS had none, so the documented mutual-decryption
  claim was only partially tested; it is now backed by committed tests in
  every port.
- CI: the C job genuinely cross-compiles the freestanding primitives to
  bare-metal ARM (new `freestanding-arm` target in `c/Makefile`,
  `arm-none-eabi-gcc`, Cortex-M4 Thumb, `CROSS_CC`/`CROSS_FLAGS`
  overridable) in addition to the host `-ffreestanding` check the docs had
  been describing as an ARM cross-compile.
- CI: 60-second fuzz smoke runs for the decrypt path on every relevant
  change: a new Rust job runs the cargo-fuzz `decrypt` target on nightly,
  the C job runs the `make fuzz` libFuzzer harness, and the C++ job builds
  and runs the `-DFUZZ=ON` harness with clang inside the Fedora container.
  A crash fails the job. Until now the fuzz harnesses existed but nothing
  ran them in CI.
...
- **Documentation audit: every claim in the landing page, project docs, and
  ...
  reality were corrected** (with the code improved to meet the claim wherever
  that was possible, rather than the claim weakened; see the Added entries
  above and each port's changelog for the code side). The wording-level
  corrections: `rust/docs/overview.md`'s architecture section and threat
  model described a codebase several generations old (a `cli` feature that
  no longer exists, raw-key mode as "bare CTR by design" when it is now
  authenticated by default) and were rewritten; `SECURITY.md`'s memory and
  fuzzing claims are now scoped per implementation instead of speaking for
  all ports in Rust's voice; the landing page no longer says "from scratch
  ten ways" (nine implementations, with the TypeScript WASM shipping
  disclosed), no longer counts HMAC-SHA256 as from-scratch, and no longer
  promotes Go into the wipe-and-lock tier its own docs exclude it from;
  `rust/README.md` no longer describes the not-yet-shipped signed macOS dmgs
  as available, and its "zero runtime dependencies"/"seven crates" wording
  now matches the manifests; `cpp/README.md`'s SHA-256 vectors are correctly
  attributed to FIPS 180-4 (not RFC 8439, which is ChaCha20-Poly1305) and
  its testing section now admits the sanitizer and fuzz harnesses it
  actually has; stale six-port enumerations, the "tighten-only" description
  of `DORADO_MAX_CHUNK_BYTES` (it can raise the cap toward the 1 GiB
  ceiling too), Python's cipher described as "compiled", and the
  raw-authenticated fixture doc's wrong multichunk byte count were all
  corrected; `docs/spec.md` now states the operative 64 MiB default
  acceptance cap alongside the wire format's 2^30 field maximum.
